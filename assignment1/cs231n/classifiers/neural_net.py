...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = None
    H_r, H_c = H.shape
    correct_class_score = scores[np.arange(0, N), y].copy()
    loss_matrix = np.exp(scores)
    ...
    F = np.zeros((N, C))
    F = np.exp(scores)/ loss_vec[:, np.newaxis]
    F[np.arange(0, N), y] -= 1
    F /= N
    dW2 = (H.T).dot(F)
    dW2 += reg * W2 # H, C
    grads['W2'] = dW2
    grads['b2'] = np.sum(F, axis=0)  # 1, C

    dH = F.dot(W2.T) # N, H
    H2 = np.where(H>0, 1, 0) #N, H, derivative of relu
    dW1 = (X.T).dot(dH*H2) # D, H
    dW1 += reg * W1    
    grads['W1'] = dW1
    grads['b1'] = np.sum(dH*H2, axis=0)  # 1, H
